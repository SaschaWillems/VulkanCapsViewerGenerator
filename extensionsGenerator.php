<?php
    ...
    class CppBuilder {
        ...
        public function writeImplementation(string $file_name, string $output_dir, $extension_container) {
            ...
            $fn_calls = array_map(function ($fn) { return "    ".$fn."();"; }, $this->header_functions);

            $function_calls_features2 = array_filter($fn_calls, function ($fn) { return (stripos($fn, "readPhysicalFeatures_") !== false); });
            $impl_source = str_replace('{{implementation_features_function_calls}}', implode("\n", $function_calls_features2), $impl_source);

            $function_calls_properties2 = array_filter($fn_calls, function ($fn) { return (stripos($fn, "readPhysicalProperties_") !== false); });
            ...
            foreach ($ext_groups as $ext_group) {
                ...
                // Properties2
                $ext_arr = array_filter($extension_container->extensions, function ($ext) use ($ext_group) { return ($ext->group == $ext_group && $ext->properties2); });
                if (count($ext_arr) > 0 ) {
